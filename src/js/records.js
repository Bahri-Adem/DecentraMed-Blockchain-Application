async function  loaddocuments() {
    ...
    while(res.patient_addr != "0x0000000000000000000000000000000000000000"){
        ...
        //console.log(res.document_type);
        let Link = row.insertCell(3);
        Link.innerHTML =  "<a id='test' class='test' href='https://ipfs.io/ipfs/QmUEzLrXBftxVACXRAERkJdftja9yKmoJZaVTLQuN24L6B'>preview</a><div id='testing' class='imgContainer hide'><img src='"+res.document_link+"' alt=''/></div>"
        if(res.document_type==="Vaccination certificate / Certificat de vaccination"){
          Link.innerHTML =  "<a id='test' class='test' href='https://ipfs.io/ipfs/QmS3H4JkD3KiAMQppHgDouCP2kRTN22yTYzCXUVQHfWaAr'>preview</a><div id='testing' class='imgContainer hide'><img src='"+res.document_link+"' alt=''/></div>"
        }
        i++;
	    res = await Authcontract.methods.patient_documents(accounts[0], i).call()
    }
